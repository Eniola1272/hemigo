...
async function main() {
  ...
  await prisma.copywritingRequest.deleteMany();
  ...
  const vendor = await prisma.vendorProfile.create({ data: { ownerId: user.id, name: "Amaka's Kitchen", slug: "amaka-kitchen", category: "Food & catering", phone: "08032145670", whatsapp: "08032145670", contactEmail: "amaka@hemigo.demo", location: "14 Admiralty Way, Lekki Phase 1, Lagos", publicLocation: "Lekki Phase 1, Lagos", fulfillmentInstructions: "Please show your order confirmation at the pickup counter.", onboardingCompleted: true, bankName: "Guaranty Trust Bank", accountNumber: "0123456789", accountName: "AMAKA OKAFOR" } });
  ...
  const products = await Promise.all(productData.map(([name, slug, description, defaultPrice]) => prisma.product.create({ data: { vendorId: vendor.id, name, slug, description, type: "FOOD", defaultPrice } })));
  const now = Date.now();
  const window = await prisma.sellingWindow.create({ data: { vendorId: vendor.id, name: "Sunday Lunch Batch", slug: "sunday-lunch", description: "A proper Sunday spread, freshly made.", headline: "Sunday tastes better here.", opensAt: new Date(now - 86_400_000), closesAt: new Date(now + 86_400_000), fulfillmentAt: new Date(now + 172_800_000), allowPayLater: true, invoiceHoldMinutes: 120, status: "LIVE" } });
  ...
  const noir = await prisma.vendorProfile.create({ data: { ownerId: noirOwner.id, name: "NOIR Lagos", slug: "noir-lagos", category: "Fashion & thrift", phone: "08000000000", location: "Victoria Island, Lagos", publicLocation: "Victoria Island, Lagos", onboardingCompleted: true } });
  await prisma.vendorMember.create({ data: { userId: noirOwner.id, vendorId: noir.id, role: "OWNER" } });
  const noirProducts = await Promise.all([["Vintage Racing Tee","vintage-racing-tee",1850000],["Utility Cargo","utility-cargo",2800000],["Archive Denim","archive-denim",3200000]].map(([name,slug,defaultPrice])=>prisma.product.create({data:{vendorId:noir.id,name:String(name),slug:String(slug),description:"A hand-picked archive piece. Once it is claimed, it is gone.",type:"PHYSICAL",defaultPrice:Number(defaultPrice)}})));
  ...
  await prisma.vendorSubscription.createMany({ data: [vendor, noir].map(({ id, createdAt }) => ({ vendorId: id, status: "TRIALING", amountKobo: 100000, trialEndsAt: new Date(createdAt.getTime() + 30 * 86_400_000) })) });
