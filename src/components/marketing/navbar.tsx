  ...
  return (
    <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/90 backdrop-blur-xl">
      <div className="container-shell flex h-18 items-center justify-between">
        ...
        <div className="hidden items-center gap-3 md:flex">{user&&<Link href="/messages" aria-label="Messages" title="Messages" className="focus-ring grid size-10 place-items-center rounded-full text-slate-500 transition hover:bg-slate-100 hover:text-indigo-700"><Bell size={20}/></Link>}{actions}</div>
      ...
      {open && <div className="border-t border-slate-200 bg-white px-5 py-5 md:hidden"><nav className="grid gap-1">{mobileLinks.map(([label, href]) => <a onClick={() => setOpen(false)} key={href} href={href} className="rounded-lg px-3 py-3 font-medium text-slate-700 hover:bg-slate-50">{label}</a>)}<div className="mt-3 flex gap-2">{actions}</div></nav></div>}
    </header>
  );
}
