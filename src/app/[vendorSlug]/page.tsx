...
type PageProps = { params: Promise<{ vendorSlug: string }> };

export async function generateMetadata({params}:PageProps):Promise<Metadata>{
  const{vendorSlug}=await params;
  const window=await db.sellingWindow.findFirst({where:{vendor:{slug:vendorSlug},mode:"SHOP",status:{not:"DRAFT"}},include:{vendor:true,windowProducts:{where:{product:{active:true}},include:{product:true},take:1}},orderBy:{createdAt:"desc"}});
  if(!window)return{title:"Shop not found",robots:{index:false}};
  const title=`${window.vendor.name} online shop`;
  const description=(window.headline||window.description||`Shop products from ${window.vendor.name} on Hemigo.`).slice(0,160);
  const path=`/${vendorSlug}`;
  const image=window.windowProducts[0]?.product.imageUrl||siteConfig.logo;
  return{title,description,alternates:{canonical:path},openGraph:{title,description,url:path,type:"website",locale:"en_NG",siteName:"Hemigo",images:[{url:image}]},twitter:{card:"summary_large_image",title,description,images:[image]}};
}
...
export default async function VendorShopPage({params}:{params:Promise<{vendorSlug:string}>}){
  ...
  const url=absoluteUrl(`/${window.vendor.slug}`);
  const structuredData={"@context":"https://schema.org","@type":"Store",name:window.vendor.name,url,description:data.description,areaServed:data.location||"Nigeria",makesOffer:data.products.map(product=>({"@type":"Offer",url,priceCurrency:"NGN",price:(product.priceKobo/100).toFixed(2),availability:data.isOpen&&(product.inventory===null||product.sold+product.reserved<product.inventory)?"https://schema.org/InStock":"https://schema.org/OutOfStock",itemOffered:{"@type":"Product",name:product.name,description:product.description,image:product.image,category:product.type}}))};
