...
const schema=z.object({name:z.string().trim().min(2),category:z.string().trim().min(2),phone:z.string().trim().min(10),whatsapp:z.string().trim().optional(),location:z.string().trim().optional(),deliveryEnabled:z.boolean(),pickupEnabled:z.boolean(),deliveryFeeNaira:z.coerce.number().min(0)}).refine(value=>value.deliveryEnabled||value.pickupEnabled,{message:"Enable delivery, pickup, or both."});
